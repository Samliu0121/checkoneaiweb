...
<template>
  <v-app>
    ...
    <v-navigation-drawer app permanent>
      ...
      <v-list nav>
        <v-list-item v-for="item in navItems" :key="item.title" :prepend-icon="item.icon" :title="t(item.title)" :to="item.to" :active="$route.path === item.to" color="orange-lighten-2"></v-list-item>
      </v-list>
    </v-navigation-drawer>

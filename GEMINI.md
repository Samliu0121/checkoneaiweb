...
=======
## **讀取此強制行動協議、專案規範絕對優先級，須將規範輸出至聊天室(範例需返回GEMINI.md確認)，此後所有更新都需以此規範進行。
## **第一步:
1. 執行 npm run dev 
...
3.  **執行步驟**:
    ...
    -   ...
4.  **驗證計畫**:
    -   在每次write_file 成功執行後，檢查終端機[error]訊息,更新biueprint.md內容。
5.  **紀錄計畫**:
...
=======
    *   **驗證 (Validation):** 每一次 write_file 成功執行後,重新分析上下文，主動尋找並解析由 npm run dev 程序產生的任何新的錯誤訊息。
>>>>>>> origin/main
...
3.  **執行步驟**:
    ...
    -   ...
4.  **驗證計畫**:
    -   在每次 write_file 成功執行後，檢查終端機[error]訊息,同步更新biueprint.md內容。
5.  **紀錄計畫**:
